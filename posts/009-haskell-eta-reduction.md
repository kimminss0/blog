...
updated: 2025-11-22T01:00:00+09:00
---

> **요점**: η-reduction을 남발하면 가독성 떨어지는데, 적절하게 쓰면 깔끔하다.

**η-reduction**은 함수의 인자를 명시하지 않고 함수를 표현하는 방법이다. 예를 들어, `f x = g x`는 간단하게 `f = g`로 표현할 수 있다. 주로 함수형 프로그래밍에서 함수의 간결한 작성과 가독성을 위해 자주 사용한다.
...
위의 예시에서는 원본 함수가 pointful, eta-reduction 적용한 1, 2번이 pointfree 스타일이다. 그런데 pointful한 원본이 가장 읽기 쉽지 않나? 이 예시에서는 pointfree 스타일이 *pointless*[^1]하다. 그냥 아래처럼 명확한 경우만 η-reduction 써야겠다.

[^1]: pointless: 무의미한, 할 가치가 없는
